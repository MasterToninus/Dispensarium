...
\end{observation}
	...
	\begin{Warning}
	(Advances)\\
	It is also noteworthy that, given any two vector bundles $F =F(M,\pi,V)$ and $F' =F'(M,\pi',V')$, we can construct naturally a third fiber bundle, the bundle of morphisms $Mor(F,F')$ over the base space $M$. 
	Its fiber over a base point $p\in M$ is $Mor(F_p,F'_p)$, which is a vector space isomorphic to the vector space Hom(V,V?) of homomorphism fromV toV?. If F? =F, then
we shall write End(F) for Hom(F,F?) and call it bundle of endomorphism, whose typical fiber is End(V).
	\end{Warning}
...
In the context of Classical mechanics is customary to refer to the cotangent bundle $T^*Q$ over the smooth manifold  $Q $ - called \emph{Configuration Space}  - as \emph{Phase Space}.\cite{}
	...
	\begin{proposition}[Canonical symplectic form]
			...
			$d \theta_0$ is a symplectic 2-form on $M$.
	\end{proposition}
	\begin{proof}
